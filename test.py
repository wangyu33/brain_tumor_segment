...
def main(mode):
    path_name = '“Jiu0Monkey”_NestedUNet_woDS'
    args = joblib.load('models/%s/args.pkl' %path_name)

    if not os.path.exists('output/%s' %path_name):
        os.makedirs('output/%s' %path_name)
    if not os.path.exists('output/%s/mask' %path_name):
        os.makedirs('output/%s/mask' %path_name)
    ...
    # Data loading code
    img_paths = glob(r'D:\brain_tumor_segment\testImage\*')
    mask_paths = glob(r'D:\brain_tumor_segment\testMask\*')
    ...
    model.load_state_dict(torch.load('models/%s/model.pth' %path_name))
    model.eval()
    ...
    if mode == "Calculate":
        ...
        maskPath = glob("output/%s/" % path_name + "GT\*.png")
        pbPath = glob("output/%s/" % path_name + "*.png")
        if len(maskPath) == 0:
            print("请先生成图片!")
            return
